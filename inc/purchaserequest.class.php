...
class PluginPurchaserequestPurchaseRequest extends CommonDBTM {
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate = 0) {
      ...
      if ($item->getType() == "PluginPurchaserequestPurchaseRequest") {
         return __('Approval');
      } else if ($item->getType() == "Ticket" && Session::getCurrentInterface() == 'central') {
   ...
   public function post_addItem() {
      ...
      if ($threshold->getFromDBByCrit(["itemtype" => $itemtype,
   ...
   function post_updateItem($history = 1) {
      ...
      if (isset($this->oldvalues['users_id_validate'])) {
         $validation = new PluginPurchaserequestValidation();
         $validation->deleteByCriteria(["users_id_validate" => $this->oldvalues['users_id_validate'], "plugin_purchaserequest_purchaserequests_id" => $this->fields["id"]]);
         $input                                               = [];
         $input["entities_id"]                                = $this->fields["entities_id"];
         $input["users_id"]                                   = $this->fields["users_id_creator"];
         $input["plugin_purchaserequest_purchaserequests_id"] = $this->fields["id"];
   ...
   function checkMandatoryFields($input) {
      ...
      $mandatory_fields = ['users_id'          => __('Requester'),
   ...
   public function rawSearchOptions() {
      ...
      $tab[] = [
         'id'        => 2,
         'table'     => $dbu->getTableForItemType('User'),
         'field'     => 'name',
         'name'      => __("Requester"),
         'linkfield' => 'users_id',
         'datatype'  => 'dropdown'
      ...
      $tab[] = [
         'id'        => 11,
         'table'     => $dbu->getTableForItemType('Group'),
         'field'     => 'name',
      ...
      $tab[] = [
         'id'        => 12,
         'table'     => $dbu->getTableForItemType('PluginPurchaserequestPurchaseRequestState'),
         'field'     => 'name',
         'name'      => __("Status"),
   ...
   static function showFormPurchase($tickets_id) {
      ...
      if ($count == 1 && $actor->getFromDBByCrit(["`tickets_id` = $tickets_id AND `type` = " . CommonITILActor::REQUESTER])) {
         $purchaserequest->fields['users_id'] = $actor->fields['users_id'];
      }

      echo "<form name='form' method='post' action='" . Toolbox::getItemTypeFormURL('PluginPurchaserequestPurchaseRequest') . "'>";
      ...
      echo "<tr class='tab_bg_1'><td>" . __("Location") . "&nbsp;</td>";
      echo "<td>";
      Dropdown::show('Location', ['value'  => $purchaserequest->fields["locations_id"],
                                  'entity' => $purchaserequest->fields["entities_id"]]);
      echo "</td>";
      echo "<td>" . __("Status") . "&nbsp;</td>";
      echo "<td>";
      ...
      echo "<tr class='tab_bg_1'><td>" . __("Description") . "&nbsp;<span style='color:red;'>*</span></td>";
      echo "<td colspan='3'>";
      $opt = ["id" => "comment", "name" => "comment", "row" => 4, "cols" => 100, "enable_richtext" => true, "value" => Toolbox::stripslashes_deep($purchaserequest->fields['comment'])];
      Html::textarea($opt);
   ...
   static function showValidation($item) {
      ...
      echo "<form name='form' id='formvalidation' method='post' action='" . Toolbox::getItemTypeFormURL('PluginPurchaserequestPurchaseRequest') . "'>";
      ...
      if ($validator && $item->fields["status"] == CommonITILValidation::WAITING) {
         ...
         } else if ($item->fields['status'] == CommonITILValidation::REFUSED) {
   ...
   function getSpecificMassiveActions($checkitem = null) {
      ...
      if (self::canValidation()) {
         $actions['PluginPurchaserequestPurchaseRequest:validate'] = __("Validate purchasing requests", "purchaserequest");
      }
   ...
   static function getHistoryEntry($data) {
      ...
      switch ($data['linked_action'] - Log::HISTORY_PLUGIN) {
         case self::HISTORY_ADDLINK :
            return sprintf(__('%1$s: %2$s'), __('Add a link with an item'),
                           $data["new_value"]);

         case self::HISTORY_DELLINK :
            return sprintf(__('%1$s: %2$s'), __('Delete a link with an item'),
                           $data["old_value"]);
